  ...
  buildHeaderValue() {
    ...
    n('header_value_start')
      .otherwise(this.load('header_state', {
        ...
        [HEADER_STATE.CONTENT_LENGTH]: n('header_value_content_length'),
    ...
    if (this.isStrict) {
      n('headers_almost_done')
        .peek('\r', n('headers_almost_done_checked'))
        .otherwise(p.error(ERROR.STRICT, 'Expected LF after headers'));
    } else {
      n('headers_almost_done')
        .otherwise(n('headers_almost_done_checked'));
    }

    // TODO(indutny): trailing headers
    n('headers_almost_done_checked')
