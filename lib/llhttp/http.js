...
class HTTP {
  ...
  emitMessageComplete(next) {
    return this.p.invoke(this.callback.onMessageComplete, {
      [ERROR.PAUSED]: this.pause('message_complete pause', next)
    }, this.node(next));
  }
  ...
  buildHeaderValue() {
    ...
    const afterHeadersComplete = p.invoke(callback.afterHeadersComplete, {
      1: this.emitMessageComplete(pauseAfterConnect),
    ...
    afterHeadersComplete
      .otherwise(this.emitMessageComplete('restart'));
