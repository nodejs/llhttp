...
class HTTP {
  constructor(p, isStrict = false) {
    ...
    this.callback = {
      prepareHeadersComplete:
        p.code.match('http_parser__prepare_headers_complete'),
  ...
  buildHeaderValue() {
    ...
    const prepareHeadersComplete =
      p.invoke(this.callback.prepareHeadersComplete);
    ...
    checkEncConflict.otherwise(prepareHeadersComplete);
    prepareHeadersComplete.otherwise(onHeadersComplete);
