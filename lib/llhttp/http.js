...
class HTTP {
  ...
  buildHeaderValue() {
    ...
    const checkEncConflict = this.testFlag(ENCODING_CONFLICT, {
    ...
    // TODO(indutny): write C code to set `upgrade` property
     ...
     */
    // TODO(indutny): handle return value of actual user callback
    // (Set `upgrade`, `SKIPBODY`)
    checkEncConflict
      .otherwise(p.invoke(callback.onHeadersComplete, {
        // TODO(indutny): pause
        0: n('headers_done_cr')
    ...
    // TODO(indutny): SKIPBODY
    n('headers_done').otherwise(n('restart'));
  }
}
module.exports = HTTP;
