...
class HTTP {
  constructor(p, isStrict = false) {
    ...
    this.callback = {
      onSpecificHeader: p.code.value('http_parser__on_specific_header')
    };
  ...
  build() {
    ...
    n('header_field')
      ...
      .select(
        ...
        p.invoke(this.callback.onSpecificHeader)
    ...
    n('header_field_general')
      ...
      .peek(
        ':',
        this.span.headerField.end().skipTo(n('header_value_discard_ws'))
      )
      .otherwise(p.error(ERROR.INVALID_HEADER_TOKEN, 'Invalid header token'));
