...
module.exports = (p, isStrict = false) => {
  ...
  const onMethod = p.invoke(p.code.value('http_parser__on_method'), {
    0: reqSpacesBeforeUrl
  }, p.error(ERROR.INTERAL, 'http_parser__on_method returned invalid value'));

  const onURLStart = p.invoke(p.code.match('http_parser__on_url_start'), {
    // Normal METHODS
    0: url.start,
  ...
  const onURLEnd = p.invoke(p.code.match('http_parser__on_url_end'), {
    0: httpStart
  ...
  const onMajor = p.invoke(p.code.value('http_parser__on_major'), {
    0: httpDot
  }, p.error(ERROR.INTERNAL, 'http_parser__on_major returned invalid value'));

  const onMinor = p.invoke(p.code.value('http_parser__on_minor'), {
    0: headerFieldStart
  ...
  startReq
    .match('\r', startReq)
    .match('\n', startReq)
    .select(constants.METHODS, onMethod)
