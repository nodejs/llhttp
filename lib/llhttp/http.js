...
class HTTP {
  ...
  mulAddContentLength(targets, base = 10) {
    const p = this.p;
    const options = { base, signed: false };
  ...
  buildHeaderValue() {
    ...
    const onHeadersComplete = p.invoke(callback.onHeadersComplete, {
      ...
      3: this.pause('paused by on_headers_complete', 'headers_done')
    ...
    afterHeadersComplete
      .otherwise(this.invokePausable('on_message_complete', 'restart'));
    ...
    n('chunk_size')
      .select(constants.HEX_MAP, this.mulAddContentLength({
        success: 'chunk_size',
    ...
    /* just ignore this. TODO(indutny): check for overflow */
    n('chunk_parameters')
      .match('\r', n('chunk_size_almost_done'))
    ...
    const toChunk = this.isEqual('content_length', 0, {
      // TODO(indutny): implement trailing support above
